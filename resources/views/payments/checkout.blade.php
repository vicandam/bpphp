...
<div class="container mt-8 mx-auto flex flex-col items-center gap-4">
    <header class="text-sm">
        ...
        <p class="text-sm text-gray-600 text-center">
            We accept Visa, MasterCard, GCash, Maya, and more
        </p>
        <div class="flex flex-wrap justify-center gap-3 pt-2">
            <img src="{{asset('images/visa-colored.png')}}" alt="Visa" class="h-6" />
            <img src="{{asset('images/master-colored.png')}}" alt="MasterCard" class="h-7" />
            <img src="https://static.xendit.co/logos/new-logos/gcash-logo.svg" alt="GCash" class="h-7" />
            <img src="https://static.xendit.co/logos/new-logos/paymaya-logo.svg" alt="Maya" class="h-7" />
        </div>
    </header>
    ...
    <div class="mt-8 flex w-[500px] flex-col rounded-md border border-gray-300 bg-white">
        ...
        @if(app()->environment('local', 'development'))
            @include('payments.fields_local')
        @else
            @include('payments.fields_prod')
        @endif
        ...
        >
            ...
            <!-- E-Wallet Selection -->
            <div class="col-span-6 mb-4 flex justify-center gap-6">
                <button onclick="selectEwallet('PH_GCASH', this)" class="wallet-btn">
                    <img src="{{asset('images/payments/gcash.png')}}" alt="GCash" class="h-12 w-auto hover:scale-110 transition-transform">
                </button>
                <button onclick="selectEwallet('PH_PAYMAYA', this)" class="wallet-btn">
                    <img src="{{asset('images/payments/paymaya.png')}}" alt="Maya" class="h-12 w-auto hover:scale-110 transition-transform">
                </button>
                <button onclick="selectEwallet('PH_GRABPAY', this)" class="wallet-btn">
                    <img src="{{asset('images/payments/grabpay.png')}}" alt="Maya" class="h-12 w-auto hover:scale-110 transition-transform">
                </button>
                <button onclick="selectEwallet('PH_SHOPEEPAY', this)" class="wallet-btn">
                    <img src="{{asset('images/payments/shopeepay.png')}}" alt="Maya" class="h-12 w-auto hover:scale-110 transition-transform">
                </button>
            </div>

            <input type="hidden" id="selected_wallet_input" name="ewallet" value="" />
            ...
            <button
                ...
                class="col-span-6 text-sm uppercase rounded-md bg-black py-3 font-bold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50 hidden"
            >
                Charge with eWallet
            </button>


            <p class="col-span-6 text-center text-xs text-gray-500 italic mt-2 mx-auto leading-tight">
...
<script>
    ...
    const ticketPrice = @json($ticketPrice);
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // E-Wallet selection
        window.selectEwallet = function (value, el) {
            selectedEwallet = value;

            // Show text fields
            const fields = document.getElementById('ewallet-fields');
            fields.classList.remove('hidden');


            // Show charge button and add image + update text
            const chargeBtn = document.getElementById('charge-ewallet-btn');
            if (chargeBtn) {
                chargeBtn.classList.remove('hidden');

                const imageMap = {
                    PH_GCASH: { src: '/images/payments/gcash.png', label: 'GCash' },
                    PH_PAYMAYA: { src: '/images/payments/paymaya.png', label: 'Maya' },
                    PH_GRABPAY: { src: '/images/payments/grabpay.png', label: 'GrabPay' },
                    PH_SHOPEEPAY: { src: '/images/payments/shopeepay.png', label: 'ShopeePay' },
                };

                const selected = imageMap[value];

                // Update or insert image
                let img = chargeBtn.querySelector('img');
                if (!img) {
                    img = document.createElement('img');
                    img.className = 'h-5 inline-block mr-2';
                    chargeBtn.prepend(img);
                }
                img.src = selected.src;
                img.alt = selected.label + " Logo";

                // Update text content
                chargeBtn.lastChild.textContent = `Charge with ${selected.label}`;
            }
        };
        ...
        function getChannelProperties(channelCode) {
            const baseProps = {
                success_redirect_url: 'https://bpphp.fun/thank-you',
                failure_redirect_url: `${BASE_URL}/ewallet/failed`
            }

            if (channelCode === 'PH_PAYMAYA' || channelCode === 'PH_SHOPEEPAY') {
                return {
                    ...baseProps,
                    cancel_redirect_url: `${BASE_URL}/checkout`
                }
            }
        ...
        function chargeEwallet() {
            if (!selectedEwallet) {
                alert("Please select an e-wallet before proceeding.");
                return;
            }
            ...
            axios.post('/pay-via-ewallet', {
                ...
                channel_properties: getChannelProperties(selectedEwallet),
                metadata: {
                ...
                .then(response => {
                    ...
                    const channelCode = response.data.channel_code; // Detect what e-wallet was used

                    let redirectUrl = actions.desktop_web_checkout_url
                        || actions.mobile_web_checkout_url
                        || actions.mobile_deeplink_checkout_url;

                    if (redirectUrl) {
                        window.location.href = redirectUrl;
                    } else if (channelCode === 'PH_SHOPEEPAY' && actions.qr_checkout_string) {
                        alert("ShopeePay does not support desktop checkout. Please scan this QR in your Shopee app:\n\n" + actions.qr_checkout_string);
                        // Optional: show modal with rendered QR code
                    } else {
                        alert("Unable to redirect. No valid checkout URL found.");
                    }
                })
                .catch(error => {
                    const err = JSON.parse(error.response?.data?.message || '{}');
                    console.error('ShopeePay Error:', err);
