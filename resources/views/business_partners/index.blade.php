                ...
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center">
                        ...
                        @if(Auth::user() && (Auth::user()->is_admin || Auth::user()->is_marketing_agent || Auth::user()->is_marketing_catalyst))
                            <a href="{{ route('business_partners.create') }}" class="btn btn-white btn-sm mb-0 me-3">Add New Partner</a>
                        @endif
                    </div>
                </div>
                                ...
                                @foreach($businessPartners as $partner)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <a href="{{ route('business_partners.show', $partner) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="View Partner">
                                                View
                                            </a>
                                            @if(Auth::user() && (Auth::user()->is_admin || Auth::user()->is_marketing_agent || Auth::user()->is_marketing_catalyst))
                                                <a href="{{ route('business_partners.edit', $partner) }}" class="text-secondary font-weight-bold text-xs ms-3" data-toggle="tooltip" data-original-title="Edit Partner">
                                                    Edit
                                                </a>
                                                <form action="{{ route('business_partners.destroy', $partner) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-link text-danger text-gradient px-3 mb-0" onclick="return confirm('Are you sure you want to delete this business partner?')">Delete</button>
                                                </form>
                                            @endif
                                        </td>
                                    </tr>
